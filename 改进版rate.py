...
leiji = pd.read_excel(file_path + yesterday_format +'不良明细.xlsx')    #导入累计出货（105表）
bs = leiji[leiji['过账日期'].isin([yesterday_format11])]    #筛出昨日不良明细

#筛出累计的品差明细
leiji_quality = leiji[leiji['责任归属'].isin(['供应商责任'])]  # 筛选出供应商责任
leiji_quality = leiji_quality.reset_index(drop = True)
...
for i in ls:
    TTT1 = leiji_quality[leiji_quality['不良原因'].str.contains(i)]
    new= new.append(TTT1,ignore_index=True)
display(new)
...
YIELD_quality = YIELD_quality[YIELD_quality["退回日期"]==yesterday_format11+' 00:00:00']     #品质差错筛出昨天的数据
...
for y in range(len(list_code)):
    each = bs[bs["供应商"]==list_code[y]]   #昨日各供应商订单量
...
recent = supplier[supplier['退回日期'].isin([yesterday_format11+' 00:00:00'])]
